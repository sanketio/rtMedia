...
class RTMediaFormHandler
{
	...
	public static function checkbox( $args, $echo = true ) {
		...
		if ( $echo ) echo $chkObj->get_switch( $args ); else
	...
	public static function radio( $args ) {
		...
		if ( ! empty( $key ) ) $args[ 'name' ] = 'rtmedia-options[' . $key . ']';
	...
	public static function types_content() {
		...
		<div class="row rt-header">
			...
			<div class="columns large-4"><h4 data-tooltip class="has-tip"
											 title="<?php _e( 'Allows you to upload a particular media type on your post.', 'rtmedia' ); ?>">
					<abbr><?php _e( 'Allow Upload', 'rtmedia' ); ?></abbr></h4></div>
			<div class="columns large-4"><h4 data-tooltip class="has-tip"
											 title="<?php _e( 'Put a specific media as a featured content on the post.', 'rtmedia' ); ?>">
					<abbr><?php _e( 'Set Featured', 'rtmedia' ); ?></abbr></h4></div>
	...
	public static function sizes_content() {
		...
		foreach ( $render_data as $parent_key => $section ) {
			if ( ++$even % 2 ) echo '<div class="row rt-odd">'; else
				echo '<div class="row rt-even">';
		...
		echo '</div>';
		$options = $rtmedia->options;
		$render_video_thumb = array( 'title' => __( 'Number of thumbnails to generate on video upload', 'rtmedia' ), 'callback' => array( 'RTMediaFormHandler', 'number' ), 'args' => array( 'key' => 'general_videothumbs', 'value' => $options[ 'general_videothumbs' ], 'class' => array( 'rtmedia-setting-text-box' ), 'desc' => __( ' If you choose more than 1 thumbnail, your users will be able to change thumbnail by going to video "edit" section.', 'rtmedia' ), 'min' => 1 ) );
		?>
		<div class="postbox metabox-holder">
		...
		<div class="row section">
			...
			<div class="columns large-3">
				...
				<span data-tooltip class="has-tip"
					  title="<?php echo ( isset( $render_video_thumb[ 'args' ][ 'desc' ] ) ) ? $render_video_thumb[ 'args' ][ 'desc' ] : 'NA'; ?>"><i
						class="rtmicon-info-circle"></i></span>
			</div>
		</div>
	<?php
	...
	static function privacy_render_options( $options ) {
		...
		$render = array( 'enable' => array( 'title' => __( 'Enable privacy', 'rtmedia' ), 'callback' => array( 'RTMediaFormHandler', 'checkbox' ), 'args' => array( 'id' => 'rtmedia-privacy-enable', 'key' => 'privacy_enabled', 'value' => $options[ 'privacy_enabled' ], 'desc' => __( 'Enable privacy in rtMedia', 'rtmedia' ) ) ), 'default' => array( 'title' => __( 'Default privacy', 'rtmedia' ), 'callback' => array( 'RTMediaFormHandler', 'radio' ), 'args' => array( 'key' => 'privacy_default', 'radios' => $rtmedia->privacy_settings[ 'levels' ], 'default' => $options[ 'privacy_default' ], 'desc' => __( 'Set default privacy for media', 'rtmedia' ) ), ), 'user_override' => array( 'title' => __( 'Allow users to set privacy for their content', 'rtmedia' ), 'callback' => array( 'RTMediaFormHandler', 'checkbox' ), 'args' => array( 'key' => 'privacy_userOverride', 'value' => $options[ 'privacy_userOverride' ], 'desc' => __( 'If you choose this, user will be able to change privacy of their own uploads.', 'rtmedia' ) ), 'after_content' => __( 'For group uploads, BuddyPress groups privacy is used.', 'rtmedia' ) ) );
	...
	public static function privacy_content() {
		...
		foreach ( $render_data as $key => $privacy ) {
			...
			<?php
			echo '<div class="columns large-6">';
			if ( $key != 'enable' ) call_user_func( $privacy[ 'callback' ], array_merge_recursive( $privacy[ 'args' ], array( 'class' => array( 'privacy-driven-disable' ) ) ) ); else
				call_user_func( $privacy[ 'callback' ], $privacy[ 'args' ] );
			?>
			<span data-tooltip class="has-tip"
				  title="<?php echo ( isset( $privacy[ 'args' ][ 'desc' ] ) ) ? $privacy[ 'args' ][ 'desc' ] : 'NA'; ?>"><i
					class="rtmicon-info-circle"></i></span>
			<?php
			echo '</div>';
			echo '</div>';
	...
	public static function rtForm_do_settings_fields( $page, $section ) {
		...
		foreach ( (array)$wp_settings_fields[ $page ][ $section ] as $field ) {
			...
			if ( isset( $field[ 'args' ][ 'label_for' ] ) && ! empty( $field[ 'args' ][ 'label_for' ] ) ) call_user_func( $field[ 'callback' ], array_merge( $field[ 'args' ], array( 'label' => $field[ 'args' ][ 'label_for' ] ) ) ); else if ( isset( $field[ 'title' ] ) && ! empty( $field[ 'title' ] ) ) call_user_func( $field[ 'callback' ], array_merge( $field[ 'args' ], array( 'label' => $field[ 'title' ] ) ) ); else
