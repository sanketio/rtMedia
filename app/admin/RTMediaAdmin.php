...
if ( ! class_exists( 'RTMediaAdmin' ) ){
	...
	class RTMediaAdmin
	{
		...
		public function __construct() {
			...
			if ( isset ( $_POST[ 'rtmedia-options' ] ) ){
				if ( isset ( $_POST[ 'rtmedia-options' ][ 'general_showAdminMenu' ] ) && $_POST[ 'rtmedia-options' ][ 'general_showAdminMenu' ] == '1' ) add_action( 'admin_bar_menu', array( $this, 'admin_bar_menu' ), 100, 1 );
			...
			if ( is_admin() ){
				...
				if ( is_multisite() ) add_action( 'network_admin_edit_rtmedia', array( $this, 'save_multisite_options' ) );
			}
		...
		function rtmedia_hide_inspirebook_release_notice() {
			if ( rtmedia_update_site_option( 'rtmedia_inspirebook_release_notice', 'hide' ) ) echo '1'; else
				echo '0';
			die();
		}
		...
		function admin_bar_menu( $admin_bar ) {
			...
			$admin_bar->add_menu( array( 'id' => 'rtMedia', 'title' => 'rtMedia', 'href' => admin_url( 'admin.php?page=rtmedia-settings' ), 'meta' => array( 'title' => __( 'rtMedia', 'rtmedia' ), ), ) );
			$admin_bar->add_menu( array( 'id' => 'rt-media-dashborad', 'parent' => 'rtMedia', 'title' => __( 'Settings', 'rtmedia' ), 'href' => admin_url( 'admin.php?page=rtmedia-settings' ), 'meta' => array( 'title' => __( 'Settings', 'rtmedia' ), 'target' => '_self', ), ) );
			$admin_bar->add_menu( array( 'id' => 'rt-media-addons', 'parent' => 'rtMedia', 'title' => __( 'Addons', 'rtmedia' ), 'href' => admin_url( 'admin.php?page=rtmedia-addons' ), 'meta' => array( 'title' => __( 'Addons', 'rtmedia' ), 'target' => '_self', ), ) );
			$admin_bar->add_menu( array( 'id' => 'rt-media-support', 'parent' => 'rtMedia', 'title' => __( 'Support', 'rtmedia' ), 'href' => admin_url( 'admin.php?page=rtmedia-support' ), 'meta' => array( 'title' => __( 'Support', 'rtmedia' ), 'target' => '_self', ), ) );
			$admin_bar->add_menu( array( 'id' => 'rt-media-themes', 'parent' => 'rtMedia', 'title' => __( 'Themes', 'rtmedia' ), 'href' => admin_url( 'admin.php?page=rtmedia-themes' ), 'meta' => array( 'title' => __( 'Themes', 'rtmedia' ), 'target' => '_self', ), ) );
			$admin_bar->add_menu( array( 'id' => 'rt-media-hire-us', 'parent' => 'rtMedia', 'title' => __( 'Hire Us', 'rtmedia' ), 'href' => admin_url( 'admin.php?page=rtmedia-hire-us' ), 'meta' => array( 'title' => __( 'Hire Us', 'rtmedia' ), 'target' => '_self', ), ) );
		}
		...
		public function menu() {
			...
			$obj_encoding = new RTMediaEncoding( true );
			if ( $obj_encoding->api_key ) add_submenu_page( 'rtmedia-settings', __( 'Regenerate Thumbnail', 'rtmedia' ), __( 'Regen. Thumbnail ', 'rtmedia' ), 'manage_options', 'rtmedia-regenerate', array( $this, 'rt_regenerate_thumbnail' ) );
		...
		function rtmedia_hide_video_thumb_admin_notice() {
			if ( rtmedia_update_site_option( 'rtmedia-video-thumb-notice', 'hide' ) ) echo '1'; else
				echo '0';
			die();
		}

		function rtmedia_hide_addon_update_notice() {
			if ( rtmedia_update_site_option( 'rtmedia-addon-update-notice', 'hide' ) ) echo '1'; else
				echo '0';
			die();
		}
				...
				<div class="row bp-media-settings-boxes-container">
					<div id="bp-media-settings-boxes" class="columns large-9">
						...
						<?php if ( $option_group ){ //$option_group if ($page == "bp-media-settings") action="<?php echo $settings_url;   ?>
							...
							<?php
							if ( $page == 'rtmedia-addons' ) RTMediaAddon::render_addons( $page ); else if ( $page == 'rtmedia-support' ){
		...
		public function tab() {
			...
			/* rtMedia */
			$tabs[ ] = array( 'href' => get_admin_url( null, add_query_arg( array( 'page' => 'rtmedia-settings' ), 'admin.php' ) ), 'title' => __( 'rtMedia', 'rtmedia' ), 'name' => __( 'rtMedia', 'rtmedia' ), 'class' => ( $tab == 'rtmedia-settings' || $tab == 'rtmedia-addons' || $tab == 'rtmedia-support' || $tab == 'rtmedia-importer' ) ? $active_class : $idle_class );
		...
		public function settings_sub_tabs() {
			...
			$tabs[ 7 ] = array( 'href' => '#rtmedia-display', 'icon' => 'rtmicon-desktop', 'title' => __( 'Display', 'rtmedia' ), 'name' => __( 'Display', 'rtmedia' ), 'callback' => array( 'RTMediaFormHandler', 'display_content' ) );


			if ( class_exists( 'BuddyPress' ) ){
				$tabs[ 20 ] = array(
					'href' => '#rtmedia-bp', 'icon' => 'rtmicon-group', 'title' => __( 'rtMedia BuddyPress', 'rtmedia' ), 'name' => __( 'BuddyPress', 'rtmedia' ), 'callback' => array(
						'RTMediaFormHandler', 'buddypress_content',
					), //change it to BuddyPress Content
				);
			}

			$tabs[ 30 ] = array( 'href' => '#rtmedia-types', 'icon' => 'rtmicon-film', 'title' => __( 'rtMedia Types', 'rtmedia' ), 'name' => __( 'Types', 'rtmedia' ), 'callback' => array( 'RTMediaFormHandler', 'types_content' ) );

			$tabs[ 40 ] = array( 'href' => '#rtmedia-sizes', 'icon' => 'rtmicon-expand', 'title' => __( 'rtMedia Sizes', 'rtmedia' ), 'name' => __( 'Image Sizes', 'rtmedia' ), 'callback' => array( 'RTMediaFormHandler', 'sizes_content' ) );

			$tabs[ 50 ] = array( 'href' => '#rtmedia-privacy', 'icon' => 'rtmicon-lock', 'title' => __( 'rtMedia Privacy', 'rtmedia' ), 'name' => __( 'Privacy', 'rtmedia' ), 'callback' => array( 'RTMediaFormHandler', 'privacy_content' ) );
			$tabs[ 60 ] = array( 'href' => '#rtmedia-custom-css-settings', 'icon' => 'rtmicon-css3', 'title' => __( 'rtMedia Custom CSS', 'rtmedia' ), 'name' => __( 'Custom CSS', 'rtmedia' ), 'callback' => array( 'RTMediaFormHandler', 'custom_css_content' ) );

			$tabs = apply_filters( 'rtmedia_add_settings_sub_tabs', $tabs, $tab );

			$tabs[ ] = array( 'href' => '#rtmedia-general', 'icon' => 'rtmicon-wrench', 'title' => __( 'Other Settings', 'rtmedia' ), 'name' => __( 'Other Settings', 'rtmedia' ), 'callback' => array( 'RTMediaFormHandler', 'general_content' ) );
    ...
    GROUP BY p.post_author,pmp.meta_value order by p.post_author";
			...
			foreach ( $result as $obj ) {
				$formatted[ $obj->post_author ][ $obj->meta_value ] = array( 'image' => $obj->Images, 'video' => $obj->Videos, 'music' => $obj->Music, 'album' => $obj->Albums, );
			}
		...
		public function plugin_meta_premium_addon_link( $plugin_meta, $plugin_file, $plugin_data, $status ) {
			if ( plugin_basename( RTMEDIA_PATH . 'index.php' ) == $plugin_file ) $plugin_meta[ ] = '<a href="https://rtcamp.com/store/product-category/buddypress/?utm_source=dashboard&#038;utm_medium=plugin&#038;utm_campaign=buddypress-media" title="' . __( 'Premium Add-ons', 'rtmedia' ) . '">' . __( 'Premium Add-ons', 'rtmedia' ) . '</a>';
			...
			if ( isset( $post->post_mime_type ) ){
				...
				if ( is_array( $media_type ) && $media_type[ 0 ] == 'video' ){
					...
					if ( is_array( $thumbnail_array ) ){
						...
						$video_thumb_html .= '  </ul>';
						$form_fields[ 'rtmedia_video_thumbnail' ] = array( 'label' => 'Video Thumbnails', 'input' => 'html', 'html' => $video_thumb_html );
					}
				}
			}
		...
		function rtmedia_hide_template_override_notice() {
			...
			if ( rtmedia_update_site_option( 'rtmedia-update-template-notice-v3_5', 'hide' ) ) echo '1'; else
				echo '0';
			die();
		}
