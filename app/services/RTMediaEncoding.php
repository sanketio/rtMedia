...
class RTMediaEncoding
{
	...
	public function __construct( $no_init = false ) {
		...
		if ( is_admin() ){
			...
			if ( $this->api_key ) add_action( 'rtmedia_before_default_admin_widgets', array( $this, 'usage_widget' ) );
		}
			...
			if ( $usage_info ){
				if ( isset( $usage_info[ $this->api_key ]->status ) && $usage_info[ $this->api_key ]->status ){
					if ( isset( $usage_info[ $this->api_key ]->remaining ) && $usage_info[ $this->api_key ]->remaining > 0 ){
						if ( $usage_info[ $this->api_key ]->remaining < 524288000 && ! get_site_option( 'rtmedia-encoding-usage-limit-mail' ) ) $this->nearing_usage_limit( $usage_info ); elseif ( $usage_info[ $this->api_key ]->remaining > 524288000 && get_site_option( 'rtmedia-encoding-usage-limit-mail' ) ) update_site_option( 'rtmedia-encoding-usage-limit-mail', 0 );
						/**
						 * @todo update class_name
						 */
						if ( ! class_exists( 'RTMediaFFMPEG' ) && ! class_exists( 'RTMediaKaltura' ) ) add_filter( 'rtmedia_after_add_media', array( $this, 'encoding' ), 10, 3 );
	...
	function encoding( $media_ids, $file_object, $uploaded, $autoformat = true ) {
		foreach ( $file_object as $key => $single ) {
			...
			if ( preg_match( '/video|audio/i', $single[ 'type' ], $type_array ) && ! in_array( $single[ 'type' ], array( 'audio/mp3' ) ) && ! in_array( $type, $not_allowed_type ) ){
				...
				if ( $options_vedio_thumb == '' ) $options_vedio_thumb = 3;

				/**  fORMAT **/
				if ( $single[ 'type' ] == 'video/mp4' || $type == 'mp4' ) $autoformat = 'thumbnails';

				$query_args   = array(
					'url' => urlencode( $single[ 'url' ] ), 'callbackurl' => urlencode( trailingslashit( home_url() ) . 'index.php' ), 'force' => 0, 'size' => filesize( $single[ 'file' ] ), 'formats' => ( $autoformat === true ) ? ( ( $type_array[ 0 ] == 'video' ) ? 'mp4' : 'mp3' ) : $autoformat, 'thumbs' => $options_vedio_thumb, 'rt_id' => $media_ids[ $key ],
				);
	...
	public function bypass_video_audio( $flag, $file ) {
		if ( isset( $file[ 'type' ] ) ){
			...
			if ( in_array( $fileinfo[ 0 ], array( 'audio', 'video' ) ) ) $flag = true;
		}
	...
	public function update_usage( $key ) {
		...
		if ( ! is_wp_error( $usage_page ) ) $usage_info = json_decode( $usage_page[ 'body' ] ); else
			$usage_info = null;
	...
	public function nearing_usage_limit( $usage_details ) {
		...
		if ( $users ){
			foreach ( $users as $user ) $admin_email_ids[ ] = $user->user_email;
	...
	public function allowed_types( $types ) {
		if ( isset( $types[ 0 ] ) && isset( $types[ 0 ][ 'extensions' ] ) ){
			if ( is_rtmedia_upload_video_enabled() ) $types[ 0 ][ 'extensions' ] .= ',mov,m4v,m2v,avi,mpg,flv,wmv,mkv,webm,ogv,mxf,asf,vob,mts,qt,mpeg,x-msvideo'; //Allow all types of file to be uploded
			if ( is_rtmedia_upload_music_enabled() ) $types[ 0 ][ 'extensions' ] .= ',wma,ogg,wav,m4a'; //Allow all types of file to be uploded
		}
	...
	public function encoding_subscription_form( $name = 'No Name', $price = '0', $force = false ) {
		if ( $this->api_key ) $this->update_usage( $this->api_key );
  ...
  <p><textarea rows="3" cols="36" id="bpm-unsubscribe-note"></textarea></p>
	...
	public function usage_widget() {
		...
		if ( $usage_details && isset( $usage_details[ $this->api_key ]->status ) && $usage_details[ $this->api_key ]->status ){
			if ( isset( $usage_details[ $this->api_key ]->plan->name ) ) $content .= '<p><strong>' . __( 'Current Plan', 'rtmedia' ) . ':</strong> ' . $usage_details[ $this->api_key ]->plan->name . ( $usage_details[ $this->api_key ]->sub_status ? '' : ' (' . __( 'Unsubscribed', 'rtmedia' ) . ')' ) . '</p>';
			if ( isset( $usage_details[ $this->api_key ]->used ) ) $content .= '<p><span class="encoding-used"></span><strong>' . __( 'Used', 'rtmedia' ) . ':</strong> ' . ( ( $used_size = size_format( $usage_details[ $this->api_key ]->used, 2 ) ) ? $used_size : '0MB' ) . '</p>';
			if ( isset( $usage_details[ $this->api_key ]->remaining ) ) $content .= '<p><span class="encoding-remaining"></span><strong>' . __( 'Remaining', 'rtmedia' ) . ':</strong> ' . ( ( $remaining_size = size_format( $usage_details[ $this->api_key ]->remaining, 2 ) ) ? $remaining_size : '0MB' ) . '</p>';
			if ( isset( $usage_details[ $this->api_key ]->total ) ) $content .= '<p><strong>' . __( 'Total', 'rtmedia' ) . ':</strong> ' . size_format( $usage_details[ $this->api_key ]->total, 2 ) . '</p>';
			...
			if ( $usage_details[ $this->api_key ]->remaining <= 0 ) $content .= '<div class="error below-h2"><p>' . __( 'Your usage limit has been reached. Upgrade your plan.', 'rtmedia' ) . '</p></div>';
		} else {
	...
	public function handle_callback() {
		...
		if ( isset( $_REQUEST[ 'job_id' ] ) && isset( $_REQUEST[ 'download_url' ] ) && isset( $_POST[ 'thumbs' ] ) ){
			...
			if ( isset( $id ) && is_numeric( $id ) ){
				...
				if ( $_POST[ 'format' ] == 'thumbnails' ) die();
	...
	public function reencoding( $attachment, $autoformat = true ) {
		...
		if ( $media_type_array[ 0 ] == 'video' ){
			$file_object[ ] = array(
				'file' => $file, 'url' => $url, 'name' => $file_name, 'type' => $media_type,
			);
