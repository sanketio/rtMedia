		...
		public function sanitize( $input ) {
			...
			if ( isset( $_POST[ 'refresh-count' ] ) ){
				if ( $rtmedia_admin->update_count() ){
					if ( is_multisite() ) rtmedia_update_site_option( 'rtm-recount-success', __( 'Recounting of media files done successfully', 'rtmedia' ) ); else
					...
					if ( is_multisite() ) rtmedia_update_site_option( 'rtm-recount-fail', __( 'Recounting Failed', 'rtmedia' ) ); else
			...
			if ( ! isset( $_POST[ 'rtmedia_options' ][ 'videos_enabled' ] ) && ! isset( $_POST[ 'rtmedia_options' ][ 'audio_enabled' ] ) && ! isset( $_POST[ 'rtmedia_options' ][ 'images_enabled' ] ) ){
				if ( is_multisite() ) rtmedia_update_site_option( 'rtm-media-type', __( 'Atleast one Media Type Must be selected', 'rtmedia' ) ); else
			...
			if ( ! is_int( $input[ 'default_count' ] ) || ( $input[ 'default_count' ] < 0 ) || empty( $input[ 'default_count' ] ) ){
				if ( is_multisite() ) rtmedia_update_site_option( 'rtm-media-default-count', __( '"Number of media" count value should be numeric and greater than 0.', 'rtmedia' ) ); else
			...
			if ( is_multisite() ) rtmedia_update_site_option( 'rtm-settings-saved', __( 'Settings saved.', 'rtmedia' ) );
