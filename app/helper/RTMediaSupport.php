...
if ( ! class_exists( 'RTMediaSupport' ) ){
	...
	class RTMediaSupport
	{
		...
		public function get_support_content()
		{
			...
			$tabs[ ] = array( 'title' => __( 'Debug Info', 'rtmedia' ), 'name' => __( 'Debug Info', 'rtmedia' ), 'href' => '#debug', 'callback' => array( $this, 'debug_info_html' ) );
			if ( $this->is_migration_required() ){ //if any un-migrated media is there
				$tabs[ ] = array( 'title' => __( 'Migration', 'rtmedia' ), 'name' => __( 'Migration', 'rtmedia' ), 'href' => '#migration', 'callback' => array( $this, 'migration_html' ) );
			}
			?>
			<div id="rtm-support">
				...
				foreach ( $files as $key => $value ) {
					if ( ! in_array( $value, array( '.', '..' ) ) ){
						if ( is_dir( $template_path . DIRECTORY_SEPARATOR . $value ) ){
							...
							if ( $value != 'main.php' ) $result[ ] = $value;
						}
					}
				}
			}
		...
		/**
		 * Check migration required or not
		 *
		 * @return bool
		 */
