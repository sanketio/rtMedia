...
class RTMediaRouter
{
	...
	function is_template()
	{
		...
		if ( $return ){
			...
			if ( isset ( $wp_query->query_vars[ 'pagename' ] ) && $wp_query->query_vars[ 'pagename' ] == 'group-avatar' ) $return = false;
	...
	function template_include( $template )
	{
		...
		if ( $rt_ajax_request ) return $new_rt_template;
		if ( function_exists( 'bp_set_theme_compat_active' ) ) bp_set_theme_compat_active( apply_filters( 'rtmedia_main_template_set_theme_compat', true ) );
	...
	function rt_theme_compat_reset_post( $args = array() )
	{
		...
		if ( function_exists( 'bp_is_group' ) ){
			if ( bp_is_group() ){
				...
				if ( 'bp-default' != get_option( 'stylesheet' ) ) $dummy[ 'post_title' ] = '<a href="' . bp_get_group_permalink( groups_get_current_group() ) . '">' . bp_get_current_group_name() . '</a>';
			} else {
				$dummy[ 'post_type' ] = 'bp_member';
				if ( 'bp-default' != get_option( 'stylesheet' ) ) $dummy[ 'post_title' ] = '<a href="' . bp_get_displayed_user_link() . '">' . bp_get_displayed_user_fullname() . '</a>';
			}
		} else {
			global $rtmedia_query;
