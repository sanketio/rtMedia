...
class RTMediaQuery
{
	 ...
	 *
	 * @param bool   $args                The query arguments
	 */
	...
	function &query( $query )
	{
		...
		if ( isset ( $this->query ) && isset ( $this->query[ 'global' ] ) ){
			if ( $this->query[ 'global' ] == 'true' ){
				...
				if ( isset ( $this->query[ 'context_id' ] ) ) unset ( $this->query[ 'context_id' ] );
				if ( isset ( $this->query[ 'context' ] ) ) unset ( $this->query[ 'context' ] );
	...
	function populate_media()
	{
		...
		$this->set_privacy();
		if ( $this->is_single() ) $this->media_query[ 'id' ] = $this->action_query->id;
		...
		if ( isset ( $this->media_query[ 'context' ] ) ){
			...
			if ( $this->media_query[ 'context' ] == 'profile' ){
				...
				if ( ! $this->is_album_gallery() ) $this->media_query[ 'media_author' ] = $this->media_query[ 'context_id' ]; else
		...
		if ( $this->is_album_gallery() ){
			...
			if ( $order_by == ' ' ) $pre_media = $this->model->{$query_function} ( $context_id, ( $this->action_query->page - 1 ) * $this->action_query->per_page_media, $this->action_query->per_page_media ); else
			 ...
			 */
			if ( $order_by == ' ' ) $pre_media = $this->model->get_media( $this->media_query, ( $this->action_query->page - 1 ) * $this->action_query->per_page_media, $this->action_query->per_page_media ); else
		...
		if ( ! $pre_media ) return false; else
			return $pre_media;
	...
	function order_by()
	{
		...
		if ( isset ( $this->media_query[ 'order_by' ] ) ){
			...
			if ( $order_by == 'ratings' ) $order_by = 'ratings_average ' . $order . ', ratings_count';
		}
		$order_by .= ' ' . $order;
	...
	function populate_data()
	{
		...
		if ( $this->action_query->action == 'comments' && ! isset ( $this->action_query->id ) ) $this->media = $this->populate_comments(); else
		...
		if ( is_multisite() ){
			...
			if ( ! ( $this->action_query->action == 'comments' && ! isset ( $this->action_query->id ) ) ) $this->populate_post_data( $this->media );
		}
	}
	...
	function get_media_by_media_id( $id )
	{
		...
		foreach ( $this->media as $key => $media ) {
			if ( $media->media_id == $id ) return $key;
		}
	...
	function populate_post_data( $media )
	{
		if ( ! empty ( $media ) && is_array( $media ) ){
			 ...
			 */
			$media_post_query_args = array( 'orderby' => 'ID', 'order' => 'DESC', 'post_type' => 'any', 'post_status' => 'any', 'post__in' => array_map( array( $this, 'get_media_id' ), $media ), 'ignore_sticky_posts' => 1, 'posts_per_page' => $this->action_query->per_page_media );
