...
function rtmedia_image( $size = 'rt_media_thumbnail', $id = false, $recho = true )
{
	...
	if ( $id ){
		...
		if ( isset ( $media[ 0 ] ) ) $media_object = $media[ 0 ]; else
			return false;
	} else {
		global $rtmedia_media;
...
function rtmedia_title_input()
{
	...
	if ( rtmedia_request_action() == 'edit' ) $html .= '<input type="text" class="rtmedia-title-editor" name="' . $name . '" id="' . $name . '" value="' . $value . '">'; else
...
function rtmedia_description_input( $editor = true )
{
	...
	$html = '';
	if ( $editor ){
		if ( rtmedia_request_action() == 'edit' ) $html .= wp_editor( $value, $name, array( 'media_buttons' => false, 'textarea_rows' => 2, 'quicktags' => false ) ); else
...
function rtmedia_pagination_prev_link()
{
	...
	if ( $rtmedia_interaction && isset( $rtmedia_interaction->context ) && $rtmedia_interaction->context->type == 'profile' ){
		if ( function_exists( 'bp_core_get_user_domain' ) ) $link .= trailingslashit( bp_core_get_user_domain( $rtmedia_media->media_author ) ); else
		...
		if ( function_exists( 'bp_get_current_group_slug' ) ) $link .= $site_url . bp_get_groups_root_slug() . '/' . bp_get_current_group_slug() . '/';
	} else {
	...
	if ( isset ( $rtmedia_query->action_query->media_type ) ){
		if ( in_array( $rtmedia_query->action_query->media_type, array( 'photo', 'music', 'video', 'album', 'playlist' ) ) ) $link .= $rtmedia_query->action_query->media_type . '/';
	}
...
function is_rtmedia_album( $album_id = false )
{
	...
	global $rtmedia_query;
	if ( $rtmedia_query ) return $rtmedia_query->is_album(); else
		return false;
}
...
function rtmedia_global_album_list( $selected_album_id = false )
{
	...
	if ( $album_objects ){
		foreach ( $album_objects as $album ) {
			...
			if ( ( isset ( $rtmedia_query->media_query[ 'album_id' ] ) && ( $album_objects[ 0 ]->id != $rtmedia_query->media_query[ 'album_id' ] ) ) || ! isset ( $rtmedia_query->media_query[ 'album_id' ] ) ) $option .= '<option value="' . $album->id . '" ' . $selected . '>' . $album->media_title . '</option>';
		}
	}
...
function rtmedia_user_album_list( $get_all = false, $selected_album_id = false )
{
	...
	if ( $profile_option != '' ) $option .= "<optgroup label='" . __( 'Profile Albums', 'rtmedia' ) . " ' value = 'profile'>$profile_option</optgroup>";
	if ( $option_group != '' && class_exists( 'BuddyPress' ) ) $option .= "<optgroup label='" . __( 'Group Albums', 'rtmedia' ) . "' value = 'group'>$option_group</optgroup>";
...
{ //by default, first album in list will be selected
	...
	if ( $album_objects ){
		foreach ( $album_objects as $album ) {
			...
			if ( ! in_array( $album->id, $global_albums ) && ( ( isset ( $rtmedia_query->media_query[ 'album_id' ] ) && ( $album->id != $rtmedia_query->media_query[ 'album_id' ] ) ) || ! isset ( $rtmedia_query->media_query[ 'album_id' ] ) ) ) $option_group .= '<option value="' . $album->id . '" ' . $selected . '>' . $album->media_title . '</option>';

		}
	}
	$option = $global_option;
	if ( $option_group != '' ) $option .= "<optgroup label='" . __( 'Group Albums', 'rtmedia' ) . "' value = 'group'>$option_group</optgroup>";
	if ( $option ) return $option; else
		return false;
}
...
function rtmedia_item_select()
{
	...
	if ( $rtmedia_backbone[ 'backbone' ] ){
		...
		if ( isset ( $rtmedia_query->media_query[ 'media_author' ] ) && get_current_user_id() == $rtmedia_query->media_query[ 'media_author' ] ) echo '<input type="checkbox" class="rtmedia-item-selector" name="selected[]" value="' . rtmedia_id() . '" />';
	}
}
...
function show_rtmedia_like_counts()
{
	...
	if ( ! ( isset( $options[ 'general_enableLikes' ] ) && $options[ 'general_enableLikes' ] == 0 ) ){
		?>
		<div class='rtmedia-like-info<?php if ( $count == 0 ) echo ' hide'; ?>'><i class="rtmicon-thumbs-up"></i> <span
				class="rtmedia-like-counter-wrap"><span
					class="rtmedia-like-counter"><?php echo $count; ?></span> <?php _e( 'people like this' ); ?></span>
		</div>
	<?php
	}
...
function rtmedia_admin_premium_tab( $tabs )
{
	...
	$tabs[ ] = array( 'href' => get_admin_url( null, add_query_arg( array( 'page' => 'rtmedia-premium' ), 'admin.php' ) ), 'name' => __( 'Go PRO!', 'rtmedia' ), 'slug' => 'rtmedia-premium', 'class' => array( 'rtm-premium' ) );
