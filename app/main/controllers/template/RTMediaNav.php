...
class RTMediaNav
{
	...
	function custom_media_nav_tab() {
		...
		if ( function_exists( 'bp_is_group' ) && ! bp_is_group() ){
			...
			if ( $rtmedia->options[ 'buddypress_enableOnProfile' ] != 0 ){
				bp_core_new_nav_item( array( 'name' => RTMEDIA_MEDIA_LABEL . '<span>' . $profile_counts[ 'total' ][ 'all' ] . '</span>', 'slug' => RTMEDIA_MEDIA_SLUG, 'screen_function' => array( $this, 'media_screen' ), 'default_subnav_slug' => 'all', 'position' => $tab_position ) );
			}
		}
	...
	function admin_nav() {
		...
		if ( function_exists( 'bp_use_wp_admin_bar' ) && ! bp_use_wp_admin_bar() ) return;
		...
		if ( is_user_logged_in() ){
			...
			$wp_admin_bar->add_menu( array( 'parent' => 'my-account', 'id' => 'my-account-' . RTMEDIA_MEDIA_SLUG, 'title' => RTMEDIA_MEDIA_LABEL, 'href' => trailingslashit( get_rtmedia_user_link( get_current_user_id() ) ) . RTMEDIA_MEDIA_SLUG . '/' ) );

			if ( is_rtmedia_album_enable() ){
				$wp_admin_bar->add_menu( array( 'parent' => 'my-account-' . RTMEDIA_MEDIA_SLUG, 'id' => 'my-account-media-' . RTMEDIA_ALBUM_SLUG, 'title' => __( 'Albums', 'rtmedia' ), 'href' => trailingslashit( get_rtmedia_user_link( get_current_user_id() ) ) . RTMEDIA_MEDIA_SLUG . '/album/' ) );
			}
			...
			foreach ( $rtmedia->allowed_types as $type ) {
				if ( isset( $rtmedia->options[ 'allowedTypes_' . $type[ 'name' ] . '_enabled' ] ) ){
					if ( ! $rtmedia->options[ 'allowedTypes_' . $type[ 'name' ] . '_enabled' ] ) continue;
					$name = strtoupper( $type[ 'name' ] );
					$wp_admin_bar->add_menu( array( 'parent' => 'my-account-' . constant( 'RTMEDIA_MEDIA_SLUG' ), 'id' => 'my-account-media-' . constant( 'RTMEDIA_' . $name . '_SLUG' ), 'title' => $type[ 'plural_label' ], 'href' => trailingslashit( get_rtmedia_user_link( get_current_user_id() ) ) . RTMEDIA_MEDIA_SLUG . '/' . constant( 'RTMEDIA_' . $name . '_SLUG' ) . '/' ) );
				}
			}
		}
	}
	...
	public function sub_nav() {
		...
		foreach ( $rtmedia->allowed_types as $type ) {
			//print_r($type);
			if ( ! isset( $rtmedia->options[ 'allowedTypes_' . $type[ 'name' ] . '_enabled' ] ) ) continue;
			if ( ! $rtmedia->options[ 'allowedTypes_' . $type[ 'name' ] . '_enabled' ] ) continue;
		...
		foreach ( $media_count as $private => $ind_count ) {
			if ( $private <= $privacy ){
				foreach ( $ind_count as $type => $ind_ind_count ) {
					...
					if ( ! isset ( $total[ $type ] ) ) $total[ $type ] = 0;
	...
	function set_privacy( $profile ) {
		...
		if ( $profile === false ) $profile = $this->profile_id();
