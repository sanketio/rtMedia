...
class RTMediaUploadView
{
	...
	/**
	 * Upload nonce generator
	 *
	 * @param bool $echo
	 * @param bool $only_nonce
	 *
	 * @return string
	 */
	static function upload_nonce_generator( $echo = true, $only_nonce = false ) {
		...
		if ( $echo ){
			...
			if ( $only_nonce ) return wp_create_nonce( 'rtmedia_upload_nonce' );
			$token = array( 'action' => 'rtmedia_upload_nonce', 'nonce' => wp_create_nonce( 'rtmedia_upload_nonce' ) );
	...
	public function render( $template_name ) {
		...
		if ( $attr && is_array( $attr ) ){
			foreach ( $attr as $key => $val ) {
				?>
				<input type='hidden' id="rt_upload_hf_<?php echo sanitize_key( $key ); ?>" value='<?php echo $val; ?>'
					   name='<?php echo $key; ?>'/>
			<?php
			}
		}
		$upload_type = 'default';
		if ( isset ( $attr[ 'activity' ] ) && $attr[ 'activity' ] ) $upload_type = 'activity';
	...
	protected function locate_template( $template ) {
		...
		if ( ! $template_name ) $located = false;
