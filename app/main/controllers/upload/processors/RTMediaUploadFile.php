...
class RTMediaUploadFile
{
	...
	function id3_validate_type( $file ) {
		...
		switch ( $type ) {
			case 'video' :
				if ( ! class_exists( 'getID3', true ) ) include_once( trailingslashit( RTMEDIA_PATH ) . 'lib/getid3/getid3.php' );
				try {
					$getID3   = new getID3;
				...
				break;
			case 'audio' :
				if ( ! class_exists( 'getID3' ) ) include_once( trailingslashit( RTMEDIA_PATH ) . 'lib/getid3/getid3.php' );
				try {
					$getID3    = new getID3;
