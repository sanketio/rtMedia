...
class RTMediaPrivacy
{
	...
	function edit_media_privacy_ui( $echo = true ) {
		...
		if ( $privacy != '' ){
			if ( $echo ) echo '<div class="rtmedia-edit-privacy"><label for="privacy">Privacy : </label>' . $privacy . '</div>'; else
	...
	function select_privacy_ui( $echo = true, $select_id = false ) {
		...
		if ( ! is_rtmedia_privacy_user_overide() ) return false;

		global $rtmedia_media;
		$default = 0;
		if ( isset ( $rtmedia_media->privacy ) ) $default = $rtmedia_media->privacy; else {
		...
		if ( $echo ) echo $form->get_select( $attributes ); else
