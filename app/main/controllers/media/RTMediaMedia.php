...
class RTMediaMedia
{
	...
	static $default_object = array( 'id', 'blog_id', 'media_id', 'media_author', 'media_title', 'album_id', 'media_type', 'context', 'context_id', 'source', 'source_id', 'activity_id', 'cover_art', 'privacy', 'views', 'downloads', 'ratings_total', 'ratings_count', 'ratings_average', 'likes', 'dislikes' );
	 ...
	 *
	 * @param      $id
	 * @param bool $echo whether nonce should be echoed
	 *
	...
	static function media_nonce_generator( $id, $echo = true )
	{
		if ( $echo ){
			...
			$token = array( 'action' => 'rtmedia_media_nonce', 'nonce' => wp_create_nonce( 'rtmedia_' . $id ) );
		...
		foreach ( $attachments as $id ) {
			...
			foreach ( $custom_fields as $key => $value ) {
				...
				if ( ! is_null( $value ) ){
					...
					if ( is_wp_error( $status ) || $status == 0 ) return false;
				}
			}
		}
	...
	function add( $uploaded, $file_object )
	{
		...
		if ( ! $this->is_multisite() ) $this->add_taxonomy( $attachment_ids, $uploaded[ 'taxonomy' ] );
	...
	function delete( $id, $core = false, $delete_activity = true )
	{
		...
		if ( $media ){
			...
			if ( ! $core ) wp_delete_attachment( $media[ 0 ]->media_id, true );
	...
	function generate_post_array( $uploaded, $file_object )
	{
		...
		foreach ( $file_object as $file ) {
			$attachments[ ] = array( 'post_mime_type' => $file[ 'type' ], 'guid' => $file[ 'url' ], 'post_title' => $uploaded[ 'title' ] ? $uploaded[ 'title' ] : $file[ 'name' ], 'post_content' => $uploaded[ 'description' ] ? $uploaded[ 'description' ] : '', 'post_parent' => $album_id, 'post_author' => $uploaded[ 'media_author' ] );
		}
	...
	function insertmedia( $attachment_ids, $uploaded, $file_object /* added for file extension */ )
	{
		...
		foreach ( $attachment_ids as $id ) {
			...
			$media = array( 'blog_id' => $blog_id, 'media_id' => $id, 'album_id' => $uploaded[ 'album_id' ], 'media_author' => $attachment[ 'post_author' ], 'media_title' => $attachment[ 'post_title' ], 'media_type' => $this->set_media_type( $mime_type[ 0 ], $file_object /* added for file extension */ ), 'context' => $uploaded[ 'context' ], 'context_id' => $uploaded[ 'context_id' ], 'privacy' => $uploaded[ 'privacy' ] );
