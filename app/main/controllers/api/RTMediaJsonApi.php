...
class RTMediaJsonApi
{
	...
	function rtmedia_api_process_wp_forgot_password_request()
	{
		...
		//send email meassage
		if ( false == wp_mail( $user_email, sprintf( __( '[%s] Password Reset' ), get_option( 'blogname' ) ), $message ) ) echo $this->rtmedia_api_response_object( 'FALSE', $this->ec_server_error, $this->msg_server_error ); else {
		...
		if ( empty( $media_id ) ){
			...
			if ( ! empty( $comments ) ){
				foreach ( $comments as $comment ) {
					$my_comments[ 'comments' ][ ] = array( 'comment_ID' => $comment[ 'comment_ID' ], 'comment_content' => $comment[ 'comment_content' ], 'media_id' => $comment[ 'comment_post_ID' ] );
				}
				$my_comments[ 'user' ] = array( 'user_id' => $this->user_id, 'name' => $user_data[ 'name' ], 'avatar' => $user_data[ 'avatar' ] );
		...
		if ( ! empty( $media_like_users ) ){
			foreach ( $media_like_users as $like_details ) {
				if ( ! array_key_exists( $like_details->user_id, $media_likes[ 'user' ] ) ){
					...
					$media_likes[ 'likes' ][ ]                       = array( 'activity_time' => $like_time, 'user_id' => $like_details->user_id );
					$media_likes[ 'user' ][ $like_details->user_id ] = array( 'name' => $user_data[ 'name' ], 'avatar' => $user_data[ 'avatar' ] );
				}
			}
		}
