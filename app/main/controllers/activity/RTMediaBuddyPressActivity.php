...
class RTMediaBuddyPressActivity
{
	...
	function bp_get_activity_parent_content( $content ) {
		...
		if ( ! $parent_id = $activities_template->activity->item_id ) return false;

		// Bail if no parent content
		if ( empty ( $activities_template->activity_parents[ $parent_id ] ) ) return false;

		// Bail if no action
		if ( empty ( $activities_template->activity_parents[ $parent_id ]->action ) ) return false;
		...
		if ( ! empty ( $activities_template->activity_parents[ $parent_id ]->content ) ) $content .= ' ' . $activities_template->activity_parents[ $parent_id ]->content;
	...
	function non_threaded_comments() {
		if ( isset ( $_POST[ 'action' ] ) && $_POST[ 'action' ] == 'new_activity_comment' ){
			...
			if ( $act->type == 'rtmedia_update' ) $_POST[ 'comment_id' ] = $_POST[ 'form_id' ];
		}
	}

	/**
	 * Get posted update on groups
	 *
	 * @param $content
	 * @param $user_id
	 * @param $group_id
	...
	function bp_after_activity_post_form() {
		...
		$params = array(
			'url'             => ( isset ( $url ) && ( strpos( $url, '/media/' ) !== false ) ) ? str_replace( '/media/', '/upload/', $url ) : 'upload/', 'runtimes' => 'html5,flash,html4', 'browse_button' => 'rtmedia-add-media-button-post-update', // browse button assigned to 'Attach Files' Button.
