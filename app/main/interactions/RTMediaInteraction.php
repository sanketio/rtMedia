...
class RTMediaInteraction
{
	...
	function set_query() {
		...
		$args          = array( 'context' => $this->context->type, 'context_id' => $this->context->id );
	...
	function rtmedia_wpseo_og_image( $data ) {
		global $wp_query;
		if ( ! array_key_exists( 'media', $wp_query->query_vars ) ) return $data;
		global $rtmedia_query;
		if ( isset ( $rtmedia_query->media ) && $rtmedia_query->media && count( $rtmedia_query->media ) > 0 ){
			...
			foreach ( $rtmedia_query->media as $media ) {
				...
				if ( $img && isset ( $img[ 0 ] ) && $img[ 0 ] != '' ) echo '<meta property="og:image" content=' . esc_url( $img[ 0 ] ) . '/>';
			}
		}
	}
	...
	function rtmedia_wpseo_og_url( $url ) {
		global $wp_query;
		if ( ! array_key_exists( 'media', $wp_query->query_vars ) ) return $url;
