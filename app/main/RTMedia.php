...
class RTMedia
{
	...
	function custome_style_for_activity_image_size() {
		...
		<style>
			.rtmedia-activity-container .rtmedia-list .rtmedia-item-thumbnail, .bp_media_content img {
				max-width: <?php echo intval( $this->options['defaultSizes_photo_medium_width'] ); ?>px;
				max-height: <?php echo intval( $this->options['defaultSizes_photo_medium_height'] ); ?>px;
			}
			...
			.rtmedia-container ul.rtmedia-list li.rtmedia-list-item div.rtmedia-item-thumbnail {
				...
				line-height: <?php echo intval( $this->options['defaultSizes_photo_thumbnail_height'] ); ?>px;
			}
	...
	function set_allowed_types() {
		$allowed_types = array( 'photo' => array( 'name' => 'photo', 'plural' => 'photos', 'label' => __( 'Photo', 'rtmedia' ), 'plural_label' => __( 'Photos', 'rtmedia' ), 'extn' => array( 'jpg', 'jpeg', 'png', 'gif' ), 'thumbnail' => RTMEDIA_URL . 'app/assets/img/image_thumb.png', 'settings_visibility' => true ), 'video' => array( 'name' => 'video', 'plural' => 'videos', 'label' => __( 'Video', 'rtmedia' ), 'plural_label' => __( 'Videos', 'rtmedia' ), 'extn' => array( 'mp4' ), 'thumbnail' => RTMEDIA_URL . 'app/assets/img/video_thumb.png', 'settings_visibility' => true ), 'music' => array( 'name' => 'music', 'plural' => 'music', 'label' => __( 'Music', 'rtmedia' ), 'plural_label' => __( 'Music', 'rtmedia' ), 'extn' => array( 'mp3' ), 'thumbnail' => RTMEDIA_URL . 'app/assets/img/audio_thumb.png', 'settings_visibility' => true ) );
	...
	function set_default_sizes() {
		$this->default_sizes = array( 'photo' => array( 'thumbnail' => array( 'width' => 150, 'height' => 150, 'crop' => 1 ), 'medium' => array( 'width' => 320, 'height' => 240, 'crop' => 1 ), 'large' => array( 'width' => 800, 'height' => 0, 'crop' => 1 ) ), 'video' => array( 'activityPlayer' => array( 'width' => 320, 'height' => 240 ), 'singlePlayer' => array( 'width' => 640, 'height' => 480 ) ), 'music' => array( 'activityPlayer' => array( 'width' => 320 ), 'singlePlayer' => array( 'width' => 640 ) ), 'featured' => array( 'default' => array( 'width' => 100, 'height' => 100, 'crop' => 1 ) ) );
	...
	function set_privacy() {
		...
		$this->privacy_settings = array( 'levels' => array( 60 => __( '<strong>Private</strong> - Visible only to the user', 'rtmedia' ), 40 => __( '<strong>Friends</strong> - Visible to user\'s friends', 'rtmedia' ), 20 => __( '<strong>Logged in Users</strong> - Visible to registered users', 'rtmedia' ), 0 => __( '<strong>Public</strong> - Visible to the world', 'rtmedia' ) ) );
	...
	public function init_buddypress_options() {
		...
		$group = 0;
		if ( isset( $bp_media_options[ 'enable_on_group' ] ) && ! empty( $bp_media_options[ 'enable_on_group' ] ) ) $group = $bp_media_options[ 'enable_on_group' ]; else if ( function_exists( 'bp_is_active' ) ) $group = bp_is_active( 'groups' );
		$this->options[ 'buddypress_enableOnGroup' ] = $group;

		$activity = 0;
		if ( isset( $bp_media_options[ 'activity_upload' ] ) && ! empty( $bp_media_options[ 'activity_upload' ] ) ) $activity = $bp_media_options[ 'activity_upload' ]; else if ( function_exists( 'bp_is_active' ) ) $activity = bp_is_active( 'activity' );
	...
	public function init_site_options() {
		...
		$defaults = array( 'general_enableAlbums' => 0, 'general_enableComments' => 0, 'general_downloadButton' => ( isset( $bp_media_options[ 'download_enabled' ] ) ) ? $bp_media_options[ 'download_enabled' ] : 0, 'general_enableLightbox' => ( isset( $bp_media_options[ 'enable_lightbox' ] ) ) ? $bp_media_options[ 'enable_lightbox' ] : 0, 'general_perPageMedia' => ( isset( $bp_media_options[ 'default_count' ] ) ) ? $bp_media_options[ 'default_count' ] : 10, 'general_enableMediaEndPoint' => 0, 'general_showAdminMenu' => ( isset( $bp_media_options[ 'show_admin_menu' ] ) ) ? $bp_media_options[ 'show_admin_menu' ] : 0, 'general_videothumbs' => 2, 'general_AllowUserData' => 1 );
		...
		foreach ( $this->default_sizes as $type => $typeValue ) {
			foreach ( $typeValue as $size => $sizeValue ) {
				foreach ( $sizeValue as $dimension => $value ) {
					switch ( $type ) {
						case 'photo':
							if ( isset( $bp_media_options[ 'sizes' ][ 'image' ][ $size ][ $dimension ] ) && ! empty( $bp_media_options[ 'sizes' ][ 'image' ][ $size ][ $dimension ] ) ) $value = $bp_media_options[ 'sizes' ][ 'image' ][ $size ][ $dimension ];
							break;
						case 'video':
						case 'music':
							...
							switch ( $size ) {
								case 'activityPlayer':
									if ( isset( $bp_media_options[ 'sizes' ][ $old ][ 'medium' ][ $dimension ] ) && ! empty( $bp_media_options[ 'sizes' ][ $old ][ 'medium' ][ $dimension ] ) ) $value = $bp_media_options[ 'sizes' ][ $old ][ 'medium' ][ $dimension ];
									break;
								case 'singlePlayer':
									if ( isset( $bp_media_options[ 'sizes' ][ $old ][ 'large' ][ $dimension ] ) && ! empty( $bp_media_options[ 'sizes' ][ $old ][ 'large' ][ $dimension ] ) ) $value = $bp_media_options[ 'sizes' ][ $old ][ 'large' ][ $dimension ];
									break;
							}
							break;
					}
	...
	public function constants() {
		...
		if ( ! defined( 'RTMEDIA_IS_INSTALLED' ) ) define( 'RTMEDIA_IS_INSTALLED', 1 );


		/* Required Version  */
		if ( ! defined( 'RTMEDIA_REQUIRED_BP' ) ) define( 'RTMEDIA_REQUIRED_BP', '1.7' );
		...
		if ( ! defined( 'RTMEDIA_MEDIA_SLUG' ) ) define( 'RTMEDIA_MEDIA_SLUG', 'media' );

		if ( ! defined( 'RTMEDIA_MEDIA_LABEL' ) ) define( 'RTMEDIA_MEDIA_LABEL', __( 'Media', 'rtmedia' ) );

		if ( ! defined( 'RTMEDIA_ALL_SLUG' ) ) define( 'RTMEDIA_ALL_SLUG', 'all' );

		if ( ! defined( 'RTMEDIA_ALL_LABEL' ) ) define( 'RTMEDIA_ALL_LABEL', __( 'All', 'rtmedia' ) );

		if ( ! defined( 'RTMEDIA_ALBUM_SLUG' ) ) define( 'RTMEDIA_ALBUM_SLUG', 'album' );

		if ( ! defined( 'RTMEDIA_ALBUM_PLURAL_SLUG' ) ) define( 'RTMEDIA_ALBUM_PLURAL_SLUG', 'albums' );

		if ( ! defined( 'RTMEDIA_ALBUM_LABEL' ) ) define( 'RTMEDIA_ALBUM_LABEL', __( 'Album', 'rtmedia' ) );

		if ( ! defined( 'RTMEDIA_ALBUM_PLURAL_LABEL' ) ) define( 'RTMEDIA_ALBUM_PLURAL_LABEL', __( 'Albums', 'rtmedia' ) );

		/* Upload slug */
		if ( ! defined( 'RTMEDIA_UPLOAD_SLUG' ) ) define( 'RTMEDIA_UPLOAD_SLUG', 'upload' );

		/* Upload slug */
		if ( ! defined( 'RTMEDIA_UPLOAD_LABEL' ) ) define( 'RTMEDIA_UPLOAD_LABEL', __( 'Upload', 'rtmedia' ) );

		/* Global Album/Wall Post */
		if ( ! defined( 'RTMEDIA_GLOBAL_ALBUM_LABEL' ) ) define( 'RTMEDIA_GLOBAL_ALBUM_LABEL', __( 'Wall Post', 'rtmedia' ) );
	...
	function define_type_constants() {
		...
		foreach ( $this->allowed_types as $type ) {
			...
			if ( ! isset( $type[ 'name' ] ) || $type[ 'name' ] === '' ) continue;
			...
			if ( ! defined( 'RTMEDIA_' . $slug . '_SLUG' ) ) define( 'RTMEDIA_' . $slug . '_SLUG', $name );
			if ( ! defined( 'RTMEDIA_' . $slug . '_PLURAL_SLUG' ) ) define( 'RTMEDIA_' . $slug . '_PLURAL_SLUG', $plural );
			if ( ! defined( 'RTMEDIA_' . $slug . '_LABEL' ) ) define( 'RTMEDIA_' . $slug . '_LABEL', $label );
			if ( ! defined( 'RTMEDIA_' . $slug . '_PLURAL_LABEL' ) ) define( 'RTMEDIA_' . $slug . '_PLURAL_LABEL', $label_plural );
		}
	}
	...
	function init() {
		...
		$class_construct = array(
			'deprecated'       => true, 'interaction' => true, //'template' => false,
			'upload_shortcode' => false, 'gallery_shortcode' => false, 'upload_endpoint' => false, 'privacy' => false, 'nav' => true, 'like' => false, 'featured' => false, //'Group' => false, will be constructed after rtmedia pro classes are constructed.
			'ViewCount'        => false,
			//'query' => false,
		);
		global $rtmedia_nav;
	...
	function redirect_on_change_slug() {
		...
		if ( $old_slugs === false ) $old_slugs = array();
	...
	function check_global_album() {
		...
		if ( isset( $rtmedia_error ) && $rtmedia_error === true ) return false;
		if ( ! $global_album ){
	...
	function update_db() {
		...
		/* Current Version. */
		if ( ! defined( 'RTMEDIA_VERSION' ) ) define( 'RTMEDIA_VERSION', $update->db_version );
	...
	function enqueue_scripts_styles() {
		...
		if ( ! wp_script_is( 'rtp-foundation-js' ) ) wp_enqueue_script( 'rtp-foundation-js', RTMEDIA_BOWER_COMPONENTS_URL . 'js/foundation.min.js', array( 'jquery' ), RTMEDIA_VERSION );
	...
	function rtmedia_custom_image_sizes_choose( $sizes ) {
		$custom_sizes = array(
			'rt_media_thumbnail' => 'rtMedia Thumbnail', 'rt_media_activity_image' => 'rtMedia Activity Image', 'rt_media_single_image' => 'rtMedia Single Image', 'rt_media_featured_image' => 'rtMedia Fetured Image',
		);
		...
		if ( isset( $_REQUEST[ 'postid' ] ) ){ //For Regenerate Thumbnails Plugin
			if ( $parent_id = get_post_field( 'post_parent', $_REQUEST[ 'postid' ] ) ){
				...
				if ( $post_type == 'rtmedia_album' ){
					$sizes = array( 'rt_media_thumbnail', 'rt_media_activity_image', 'rt_media_single_image', 'rt_media_featured_image' );
				} else {
	...
	function unset_bp_media_image_sizes_details( $sizes ) {
		if ( isset( $sizes[ 'rt_media_thumbnail' ] ) ) unset( $sizes[ 'rt_media_thumbnail' ] );
		if ( isset( $sizes[ 'rt_media_activity_image' ] ) ) unset( $sizes[ 'rt_media_activity_image' ] );
		if ( isset( $sizes[ 'rt_media_single_image' ] ) ) unset( $sizes[ 'rt_media_single_image' ] );
		if ( isset( $sizes[ 'rt_media_featured_image' ] ) ) unset( $sizes[ 'rt_media_featured_image' ] );

		return $sizes;
	}

	/**
	 * Unset the buddypress media image sizes
	 *
	 * @param $sizes
	 *
	 * @return mixed
	...
	function unset_bp_media_image_sizes( $sizes ) {
		if ( ( $key = array_search( 'rt_media_thumbnail', $sizes ) ) !== false ) unset( $sizes[ $key ] );
		if ( ( $key = array_search( 'rt_media_activity_image', $sizes ) ) !== false ) unset( $sizes[ $key ] );
		if ( ( $key = array_search( 'rt_media_single_image', $sizes ) ) !== false ) unset( $sizes[ $key ] );
		if ( ( $key = array_search( 'rt_media_featured_image', $sizes ) ) !== false ) unset( $sizes[ $key ] );
...
/**
 * Get parent link of global album
 *
 * @param $id
 *
 * @return string
