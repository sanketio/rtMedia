...
class BPMediaImporter
{
	...
	static function _active( $path ) {
		...
		if ( is_plugin_active( $path ) ) return 1;
		$plugins = get_plugins();
	...
	/**
	 * Make File array from file path
	 *
	 * @param $filepath
	 *
	 * @return mixed
	...
	/**
	 * Create album with album id and album name
	 *
	...
	static function add_media( $album_id, $title = '', $description = '', $filepath = '', $privacy = 0, $author_id = false, $album_name = false ) {
		...
		if ( $files ){
			...
			if ( $bp_album_privacy == 10 ) $bp_album_privacy = 6;
