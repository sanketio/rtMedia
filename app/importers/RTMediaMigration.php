...
class RTMediaMigration
{
	...
	function __construct() {
		...
		if ( rtmedia_get_site_option( 'rt_migration_hide_notice' ) !== false ) return true;

		if ( isset ( $_REQUEST[ 'force' ] ) && $_REQUEST[ 'force' ] === 'true' ) $pending = false; else
		...
		if ( $pending > 0 ){
			if ( ! ( isset ( $_REQUEST[ 'page' ] ) && $_REQUEST[ 'page' ] == 'rtmedia-migration' ) ) add_action( 'admin_notices', array( &$this, 'add_migration_notice' ) );
		}
	}
	...
	function add_migration_notice() {
		if ( current_user_can( 'manage_options' ) ) $this->create_notice( '<p><strong>rtMedia</strong>: ' . __( 'Please Migrate your Database', 'rtmedia' ) . ' <a href=' . admin_url( 'admin.php?page=rtmedia-migration&force=true' ) . '>' . __( 'Click Here', 'rtmedia' ) . '</a>.  <a href=' . admin_url( 'admin.php?page=rtmedia-migration&hide=true' ) . ' style="float:right">' . __( 'Hide', 'rtmedia' ) . '</a> </p>' );
	}
	...
	function get_total_count() {
		global $wpdb;
		if ( function_exists( 'bp_core_get_table_prefix' ) ) $bp_prefix = bp_core_get_table_prefix(); else
			$bp_prefix = '';
	...
	function manage_album() {
		...
		if ( $stage < 2 ){
			...
			if ( $delete_ids != '' ) $wpdb->query( "delete from $wpdb->posts where ID in ({$delete_ids})" );
			if ( count( $results ) < 10 ){
				$stage = 2;
			}
			rtmedia_update_site_option( 'rtmedia-migration', $stage );
	...
	function migrate_to_new_db( $lastid = 0, $limit = 1 ) {
		...
		if ( ! isset ( $_SESSION[ 'migration_media' ] ) ) $this->get_total_count();
	...
	function migrate_single_media( $result, $album = false ) {
		...
		if ( $this->table_exists( $bp_prefix . 'bp_activity' ) && class_exists( 'BP_Activity_Activity' ) ){
			...
			foreach ( $all_activity as $activity ) {
				...
				if ( $comments ) $this->insert_comment( $media_id, $comments, $exclude );
			}
		}
		...
		if ( function_exists( 'bp_activity_get_meta' ) ) $likes = bp_activity_get_meta( $result->activity_id, 'favorite_count' ); else
			$likes = 0;
		...
		if ( $media_type != 'album' && function_exists( 'bp_core_get_user_domain' ) && $activity_data ){
			...
			if ( strpos( $activity_data->content, $replace_img ) === false ) $activity_data->content = str_replace( $last_baseurl, $replace_img, $activity_data->content );
			global $wpdb;
	...
	function importmedia( $id, $prefix ) {
		...
		if ( ! $url = get_option( 'upload_url_path' ) ){
			if ( empty ( $upload_path ) || ( 'wp-content/uploads' == $upload_path ) || ( $upload_path == $dir ) ) $url = WP_CONTENT_URL . '/uploads'; else
		...
		if ( is_multisite() && ! ( is_main_site() && defined( 'MULTISITE' ) ) ){
			...
			if ( ! rtmedia_get_site_option( 'ms_files_rewriting' ) ){
				...
				if ( defined( 'MULTISITE' ) ) $ms_dir = '/sites/' . get_current_blog_id(); else
				...
				if ( defined( 'BLOGUPLOADDIR' ) ) $dir = untrailingslashit( BLOGUPLOADDIR ); else
					$dir = ABSPATH . UPLOADS;
		...
		if ( wp_mkdir_p( $basedir . 'rtMedia/$prefix/' . $year_month ) ){
			if ( copy( $attached_file, str_replace( $basedir, $basedir . 'rtMedia/$prefix/', $attached_file ) ) ){
				...
				if ( $delete ){
					if ( file_exists( $attached_file ) ) unlink( $attached_file );

					if ( isset ( $delete_sizes ) ){
						foreach ( $delete_sizes as $delete_size ) {
							if ( file_exists( $delete_size ) ) unlink( $delete_size );
						}
					}
	...
	function insert_comment( $media_id, $data, $exclude, $parent_commnet_id = 0 ) {
		foreach ( $data as $cmnt ) {
			...
			if ( ! key_exists( strval( $cmnt->id ), $exclude ) ){
				$commentdata                    = array( 'comment_date' => $cmnt->date_recorded, 'comment_parent' => $parent_commnet_id, 'user_id' => $cmnt->user_id, 'comment_content' => $cmnt->content, 'comment_author_email' => $cmnt->user_email, 'comment_post_ID' => $media_id, 'comment_author' => $cmnt->display_name, 'comment_author_url' => '', 'comment_author_IP' => '' );
