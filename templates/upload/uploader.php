		...
		if ( isset( $attr[ 'rtmedia_simple_file_upload' ] ) && $attr[ 'rtmedia_simple_file_upload' ] == true ){
			...
			if ( ! empty ( $attr ) ){
				foreach ( $attr as $key => $value ) {
					if ( $key == 'context' ) echo '<input type="hidden" name="context" value="' . $value . '" />'; else if ( $key == 'context_id' ) echo '<input type="hidden" name="context_id" value="' . $value . '" />'; else if ( $key == 'privacy' ) echo '<input type="hidden" name="privacy" value="' . $value . '" />'; else if ( $key == 'album_id' ) echo '<input type="hidden" name="album_id" value="' . $value . '" />'; else if ( $key == 'title' ) echo '<p class="rtmedia-file-upload-p rtmedia-file-upload-title"><input type="text" name="title" /></p>'; else if ( $key == 'description' ) echo '<p class="rtmedia-file-upload-p rtmedia-file-upload-desc"><textarea name="description"></textarea></p>'; else
				...
				<form id="rtmedia-uploader-form" method="post" action="upload" enctype="multipart/form-data">
					...
					if ( ! empty ( $attr ) ){
						...
						foreach ( $attr as $key => $value ) {
							...
							if ( $key == 'context' ) echo '<input type="hidden" name="context" value="' . $value . '" />';
							if ( $key == 'context_id' ) echo '<input type="hidden" name="context_id" value="' . $value . '" />';
							if ( $key == 'privacy' ) echo '<input type="hidden" name="privacy" value="' . $value . '" />';
							if ( $key == 'album_id' ) echo '<input type="hidden" name="album_id" value="' . $value . '" />';
						}
					}
					?>

					<input type="submit" id='rtMedia-start-upload' name="rtmedia-upload"
						   value="<?php echo RTMEDIA_UPLOAD_LABEL; ?>"/>
				</form>
			</div>
		<?php
		}
