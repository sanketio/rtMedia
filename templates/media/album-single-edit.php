...
<div class="rtmedia-container rtmedia-single-container row rtmedia-media-edit">
	...
	<div class="rtmedia-edit-media-tabs rtmedia-editor-main columns large-12 small">
		<dl class="tabs" data-tab>
			<dd class="active"><a href="#details-tab"><i class='rtmicon-edit'></i><?php _e( 'Details', 'rtmedia' ); ?>
				</a></dd>
			<?php if ( ! is_rtmedia_group_album() ){ ?>
				<dd class=""><a href="#manage-media-tab"><i
							class='rtmicon-edit'></i><?php _e( 'Manage Media', 'rtmedia' ); ?></a></dd>
			<?php } ?>
			...
			<div class="content active" id="details-tab">
				<form method="post">
					...
					<input type="submit" name="submit" class='rtmedia-save-album'
						   value="<?php _e( 'Save Changes', 'rtmedia' ); ?>"/>
				</form>
			</div>
					...
					<?php if ( have_rtmedia() ){ ?>
						<form class="rtmedia-album-edit rtmedia-bulk-actions" method="post" name="rtmedia_album_edit">
							...
							<button type='button' class="select-all"
									title="<?php echo __( 'Select All Visible', 'rtmedia' ); ?>"><i
									class='rtmicon-square-o'></i></button>
							<button class="button rtmedia-move" type='button'
									title='<?php echo __( 'Move Selected media to another album.' ); ?>'><i
									class='rtmicon-move'></i> <?php _e( 'Move', 'rtmedia' ); ?></button>
							<button type="button" name="delete-selected" class="button rtmedia-delete-selected"
									title='<?php echo __( 'Delete Selected media from the album.' ); ?>'><i
									class='rtmicon-trash'></i><?php _e( 'Delete', 'rtmedia' ); ?></button>
							<div class="rtmedia-move-container">
								...
								<input type="button" class="rtmedia-move-selected" name="move-selected"
									   value="<?php _e( 'Move Selected', 'rtmedia' ); ?>"/>
							</div>
							...
							<?php
							$display = '';
							if ( rtmedia_offset() != 0 ) $display = 'style="display:block;"'; else
								$display = 'style="display:none;"';
							?>
							<a id="rtMedia-galary-prev" <?php echo $display; ?>
							   href="<?php echo rtmedia_pagination_prev_link(); ?>"><?php _e( 'Prev', 'rtmedia' ); ?></a>

							<?php
							$display = '';
							if ( rtmedia_offset() + rtmedia_per_page_media() < rtmedia_count() ) $display = 'style="display:block;"'; else
								$display = 'style="display:none;"';
							?>
							<a id="rtMedia-galary-next" <?php echo $display; ?>
							   href="<?php echo rtmedia_pagination_next_link(); ?>"><?php _e( 'Next', 'rtmedia' ); ?></a>
						</form>
					<?php } else { ?>
